...
// Opret denne fil med FlutterFire CLI:
...
class PushService {
  ...
  static Future<void> _configureForegroundListeners() async {
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      // Her kan du senere vise lokal notifikation i foreground.
  ...
  static Future<void> registerCurrentDeviceToken({bool force = false}) async {
    ...
    final lastRegistered = await storage.read(key: 'last_registered_device_token');
    ...
    await storage.write(key: 'last_registered_device_token', value: deviceToken);
  }
      ...
      child: MaterialApp(
        ...
        theme: ThemeData(
          ...
          colorScheme: ColorScheme.fromSeed(
            seedColor: const Color(0xFFF0D07A),
            brightness: Brightness.dark,
          ),
...
class _MainNavigationScreenState extends State<MainNavigationScreen> {
  ...
  Future<void> logout() async {
    try {
      ...
      // Vi rydder stadig lokalt, selv hvis server-siden fejler.
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          HomeHeroSection(isLoggedIn: isLoggedIn),
          const SizedBox(height: 16),
          const TickerSection(),
          const SizedBox(height: 16),
          if (isLoggedIn) ...[
...
class HomeHeroSection extends StatelessWidget {
  ...
  const HomeHeroSection({super.key, required this.isLoggedIn});

  @override
  Widget build(BuildContext context) {
    final bool mobile = MediaQuery.of(context).size.width < 760;
    final double heroHeight = mobile ? 650 : 500;
        ...
        child: Align(
          ...
          child: Padding(
            ...
            child: ConstrainedBox(
              ...
              child: _heroCard(context, isLoggedIn),
            ),
          ),
        ),
      ),
  ...
  Widget _heroCard(BuildContext context, bool isLoggedIn) {
    return Container(
...
class MemberNoticeCard extends StatelessWidget {
  const MemberNoticeCard({super.key});
    ...
    return Container(
      ...
      child: const Column(
        ...
        children: [
          ...
          SizedBox(height: 8),
          Text(
            'Prijavite se da vidite vijesti i događaje za članove zajednice.',
            style: TextStyle(
              color: Colors.white70,
              height: 1.45,
            ),
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                FilledButton(
                  onPressed: detailsUrl.isEmpty ? null : () {},
                  style: FilledButton.styleFrom(
                    backgroundColor: const Color(0xFFF0D07A),
                    foregroundColor: const Color(0xFF0B0F14),
                    shape: RoundedRectangleBorder(
                ...
                OutlinedButton(
                  onPressed: canRegister && !registered ? () {} : null,
                  style: OutlinedButton.styleFrom(
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(999),
                    ),
  ...
  Future<void> fetchContact() async {
    try {
      ...
      if (data['success'] == true) {
        ...
        setState(() {
          error = data['message']?.toString() ?? 'Kunne ikke hente kontaktinfo';
          loading = false;
        });
      }
    } catch (e) {
      ...
      children: [
        ...
        Card(
          child: ListTile(
            ...
            subtitle: Text(phone.isEmpty ? 'Ikke angivet' : phone),
          ),
        ),
      ],
    );
      ...
      children: [
        ...
        Card(
          child: ListTile(
            ...
            title: const Text('Medlemsstatus'),
            subtitle: Text(membershipStatus.isEmpty ? 'Ukendt' : membershipStatus),
          ),
        ),
        Card(
          child: ListTile(
            leading: const Icon(Icons.badge_outlined),
            title: const Text('Medlemstype'),
            subtitle: Text(membershipType.isEmpty ? 'Ikke angivet' : membershipType),
          ),
        ),
        Card(
          child: ListTile(
            ...
            subtitle: Text(validUntil.isEmpty ? 'Ikke angivet' : validUntil),
          ),
        ),
      ],
    );
